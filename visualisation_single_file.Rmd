...
fcs <- read.FCS("D:/Internship/data/7. aggregated files/healthy BM/all_tubes_aggregated_everything.fcs", transformation = FALSE)

#keyword(raw_ff)[c("$P8R","$P8B","$P8E")]
#colnames(fcs)
showme <- exprs(fcs)
```
...
# New column
cutoff_ki67 <- 0.2
new_col <- as.integer(exprs(fr)[, "KO"] > cutoff_ki67)

# Make it a matrix with column name
new_mat <- matrix(new_col, ncol = 1)
colnames(new_mat) <- "Ki67_pos"

# Append using fr_append_cols
fr <- fr_append_cols(fr, new_mat)
...
```{r}
#SELECT WHICH FILE YOU WANT TO VISUALIZE
raw_ff2 <- read.FCS("D:/Internship/data/3. cleaned dataset/healthy BM/Femoral head 1 tube 1_QC.fcs", transformation = FALSE)

#keyword(raw_ff2)[c("$P1R","$P1B","$P1E")]
colnames(raw_ff2)
```

```{r}
ff1 <- read.FCS("D:/Internship/data/3. cleaned dataset/healthy BM/Femoral head 1 tube 1_QC.fcs", transformation = FALSE)
ff2 <- read.FCS("D:/Internship/data/3. cleaned dataset/healthy BM/Femoral head 1 tube 2_QC.fcs", transformation = FALSE)

library(flowCore)

scale_to_reference <- function(target_fcs, reference_fcs, skip_channels = c()) {
  # Extract expression matrices and parameter info
  e_target <- exprs(target_fcs)
  e_ref    <- exprs(reference_fcs)
  
  pd_target <- pData(parameters(target_fcs))
  pd_ref    <- pData(parameters(reference_fcs))
  
  # Find common channels by name
  common_channels <- intersect(pd_target$name, pd_ref$name)
  
  for (ch_name in common_channels) {
    if (ch_name %in% skip_channels) next  # skip manually specified channels
    
    # Get column index in target and reference
    ...
    # Update recorded range in target
...
# Channels you want to skip manually (e.g., viability dye)
skip_ch <- c("Original_ID", "TIME")

# Scale target to reference
scaled_target <- scale_to_reference(ff1, ff2, skip_channels = skip_ch)
```

```{r}
fcs <- read.FCS("D:/Internship/data/7. aggregated files/aml/all_tubes_aggregated_everything.fcs", transformation = FALSE)
```
...
ggcyto(fcs, aes(x = `FSC-A`, y = `SSC-A`)) +
  geom_hex(bins = 128) +  # keep default colors, just fix limits
  labs(title = "aml: FSC-A vs SSC-A",
       x = "Forward Scatter (FSC-A)", y = "Side Scatter (SSC-A)") +
