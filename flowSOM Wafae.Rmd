...
## STEP 3: EDA AFTER PREPROCESSING
...
for (tube in tubes) {
  ...
  # Filter files that contain the tube string (case insensitive)
...
```{r}
fcs_files_dir <- "D:/Internship/data/results/"
all_fcs_files <- list.files(fcs_files_dir, pattern = "\\.fcs$", full.names = FALSE)
```

```{r}
flow_frame <- read.FCS("D:/Internship/data/results/tube 6_final.fcs")
colnames(flow_frame)
...
```{r}
library(FlowSOM)
fsom <- FlowSOM(input = flow_frame, scale = scaling, 
                colsToUse = c(6,8,9, 11:14),
                seed = seed, 
                nClus = n_meta, 
                xdim = SOM_x, 
                ydim = SOM_y)
...
ggsave(filename = paste0(dir_results, "fsom_tree_tube6.pdf"), plot = p, height = 8.5, width = 11)
```
