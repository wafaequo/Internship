...
In order to see if differences per celltype can be found between disease states, statistical testing was employed. Wilcoxon Rank-Sum Test are performed with Benferroni correction. For the effect size, cliff's delta is calculated.

*NOTE: during calculation of cliff's delta, NA was seen due to integer overflow. Thus for calculating cliff's delta, downsampling is applied to get idea.*
...
node_summary <- expr_all %>%
  ...
  summarise(across(c("Ki-67", "Bcl-2"), \(x) median(x, na.rm = TRUE)), .groups = "drop")

#save median values per node
write.csv(node_summary, "D:/Internship/FlowSOM results/nodesum.csv", row.names = FALSE)
```
...
markers <- c("Ki-67", "Bcl-2") #which markers?

#convert data to long format
df_long <- expr_all %>%
  select(manual_label, disease, all_of(markers)) %>%  # keep only relevant columns
...
run_stats <- function(data) {
  map_dfr(disease_pairs, function(pair) {
    ...
    max_sample <- 10000
    g1_s <- if(length(g1) > max_sample) sample(g1, max_sample) else g1
    g2_s <- if(length(g2) > max_sample) sample(g2, max_sample) else g2
    
    
    tibble(
      Disease_1 = pair[1],
      Disease_2 = pair[2],
      Wilcoxon_p = wilcox.test(g1, g2, exact = FALSE)$p.value, #statistical test
      Cliffs_delta = round(cliff.delta(g1_s, g2_s)$estimate, 2), #cliff's delta
...
results <- df_long %>%
  ...
  group_modify(~ run_stats(.x)) %>%
  ungroup() %>%
  mutate(
    Bonferroni_p = pmin(Wilcoxon_p * n(), 1),
    Significant = Bonferroni_p < 0.05
  )

#Save the results to a CSV file
write.csv(results, "D:/Internship/FlowSOM results/manual_label_wilcox_cliffs.csv", row.names = FALSE)
```
