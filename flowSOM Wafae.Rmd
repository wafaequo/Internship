...
devtools::install_github("tchitchek-lab/CytoBackBone")
```
...
We can use a developed R code section from the Duetz et al. (2021) paper to remove doublets from the data. This was manually adapted by: 1. adding an upper filter 2. changing the cutoff to 1 sd instead of 2 sd fot he lower limit.
...
### STEP 2.3 - CHANGING COLNAMES TO MARKERNAMES

Change column names to markers instead of fluorchrome
...
```{r}
# Assuming tube_markers list is defined as before
tube_markers <- list(
  c("FSC-A", "SSC-A", "IgG1-1", "NA1", "IgG1-2", "CD13", "CD117", "CD34", "NA2", "NA3", "HLA-DR", "CD45", "TIME", "FSC-H"),
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "CD14", "CD64", "CD13", "CD117", "CD34", "CD10", "CD11b", "HLA-DR", "CD45", "TIME"),
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "CD105", "CD123", "CD33", "CD117", "NA1", "CD71", "CD235a", "HLA-DR", "CD45", "TIME"),
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "NA1", "NA2", "CD33", "CD117", "CD36", "NA4", "NA5", "HLA-DR", "CD45", "TIME"),
  c("FSC-H", "FSC-A", "SSC-A", "CD16", "CD64", "Bcl-2", "CD13", "CD117", "CD34", "CD10", "CD14", "HLA-DR", "CD45", "TIME"),
  c("FSC-H", "FSC-A", "SSC-A", "NA1", "CD105", "Bcl-2", "CD33", "CD117", "NA2", "CD71", "CD235a", "HLA-DR", "CD45", "TIME"),
  c("FSC-H", "FSC-A", "SSC-A", "NA1", "NA2", "Bcl-2", "CD33", "CD117", "CD36", "NA3", "NA4", "HLA-DR", "CD45", "TIME")
)

# Helper function to extract tube number from the name of the FCS object
...
# Loop through the named list and rename columns
# Loop through the named list and rename columns
for (fcs_name in names(fcs_final)) {
  ...
  if (length(cols_keep_present) > 0) {
    ...
    # Rebuild flowFrame so metadata matches expression matrix
  ...
  # Now check column count matches the markers vector
...
## STEP 3: IMPUTATION/MERGING OF PANELS

I want to merge the files and the intention was to use CytoBackBone algorithm. However it was not compatible with my version of R and bioconductor and I tried to downgrade. However even by doing to, I ran into alot of issues. So my intention is to code it from scratch
...
get_patient_id <- function(fname) {
  # Extract substring before 'tube' (case insensitive, allow spaces)
  pat <- regexpr("tube", fname, ignore.case = TRUE)
  if (pat == -1) {
    # 'tube' not found, return filename without extension
    return(tools::file_path_sans_ext(fname))
  }
  # Extract substring before 'tube' minus space (pat - 2)
  patient_id <- substr(fname, 1, pat - 2)
  trimws(patient_id)  # remove trailing spaces
}

# Get patient IDs for each loaded FCS (names are filenames)
patient_ids <- sapply(names(fcs_preproc), get_patient_id)

# Split fcs_preproc list by patient_id
...
```{r}
CombineFCS <- function (FCS1, RelevantMarkers1, FCS2, RelevantMarkers2, arcsinhTrans = FALSE, manual_shared = c("CD117", "HLA-DR", "CD45")) {
  library(flowCore)
  library(FNN)

  # Convert marker names to indices if needed
  ...
  # Subset and convert to data.frames
  ...
  # Get indices of manually shared markers
  Shared_Index1 <- match(manual_shared, VarNames1)
  Shared_Index2 <- match(manual_shared, VarNames2)

  if (any(is.na(Shared_Index1)) || any(is.na(Shared_Index2))) {
    stop("Not all specified shared markers found in both files.")
  }

  # Remove unintended shared markers to avoid duplication
  all_shared <- intersect(VarNames1, VarNames2)
  unintended_shared <- setdiff(all_shared, manual_shared)

  VarNames1.nonshared <- setdiff(setdiff(VarNames1, manual_shared), unintended_shared)
  VarNames2.nonshared <- setdiff(setdiff(VarNames2, manual_shared), unintended_shared)

  Data1.nonshared <- FCS1.data[, VarNames1.nonshared, drop = FALSE]
  Data2.nonshared <- FCS2.data[, VarNames2.nonshared, drop = FALSE]

  # Reorder data: shared markers first
  FCS1.data <- cbind(FCS1.data[, Shared_Index1, drop = FALSE], Data1.nonshared)
  FCS2.data <- cbind(FCS2.data[, Shared_Index2, drop = FALSE], Data2.nonshared)

  VarNames1 <- c(manual_shared, VarNames1.nonshared)
  VarNames2 <- c(manual_shared, VarNames2.nonshared)

  m <- length(manual_shared)
  ...
  total_cols <- ncol(FCS1.data) + ncol(FCS2.data) - m
  Data.combine.1 <- matrix(0, nrow = nrow(FCS1.data), ncol = total_cols)
  Data.combine.2 <- matrix(0, nrow = nrow(FCS2.data), ncol = total_cols)

  for (i in seq_len(nrow(FCS1.data))) {
    Data.combine.1[i, 1:m] <- as.numeric(FCS1.data[i, 1:m])
    Data.combine.1[i, (m + 1):(m + ncol(Data1.nonshared))] <- as.numeric(FCS1.data[i, (m + 1):ncol(FCS1.data)])
    Data.combine.1[i, (m + ncol(Data1.nonshared) + 1):total_cols] <- apply(FCS2.data[IDX1[i, ], (m + 1):ncol(FCS2.data), drop = FALSE], 2, median)
  }

  for (i in seq_len(nrow(FCS2.data))) {
    Data.combine.2[i, 1:m] <- as.numeric(FCS2.data[i, 1:m])
    Data.combine.2[i, (m + 1):(m + ncol(Data1.nonshared))] <- apply(FCS1.data[IDX2[i, ], (m + 1):ncol(FCS1.data), drop = FALSE], 2, median)
    Data.combine.2[i, (m + ncol(Data1.nonshared) + 1):total_cols] <- as.numeric(FCS2.data[i, (m + 1):ncol(FCS2.data)])
  }

  Data.combine <- rbind(Data.combine.1, Data.combine.2)
  VarNames.combine <- c(manual_shared, VarNames1.nonshared, VarNames2.nonshared)
...
```{r}
marker_list <- list(
  c("FSC-A", "SSC-A", "IgG1-1", "IgG1-2", "CD13", "CD117", "CD34", "HLA-DR", "CD45","FSC-H"),  # Tube 1 markers
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "CD14", "CD64", "CD13", "CD117", "CD34", "CD10", "CD11b", "HLA-DR", "CD45"),  # Tube 2 markers
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "CD105", "CD123", "CD33", "CD117", "CD71", "CD235a", "HLA-DR", "CD45"),
  c("FSC-H", "FSC-A", "SSC-A","Ki-67","CD33", "CD117", "CD36","HLA-DR", "CD45"),
  c("FSC-H", "FSC-A", "SSC-A", "CD16", "CD64", "Bcl-2", "CD13", "CD117", "CD34", "CD10", "CD14", "HLA-DR", "CD45"),
  c("FSC-H", "FSC-A", "SSC-A", "CD105", "Bcl-2", "CD33", "CD117", "CD71", "CD235a", "HLA-DR", "CD45"),
  c("FSC-H", "FSC-A", "SSC-A", "Bcl-2", "CD33", "CD117", "CD36","HLA-DR", "CD45")
)

fcs <- read.FCS('D:/Internship/data/6. merged dataset/healthy BM/Femoral head 1_merged.fcs')

df<- as.data.frame(fcs@exprs)

fcs2 <- read.FCS('D:/Internship/data/5. preprocessed dataset/healthy BM/Femoral head 1 tube 4_QC.fcs')

merged_data <- CombineFCS(FCS1=fcs, RelevantMarkers1=colnames(fcs), FCS2=fcs2, RelevantMarkers2=marker_list[[4]])
```
...
marker1 <- colnames(fcs)
marker2 <- marker_list[[4]]

selected1 <- marker1
selected2 <- marker2

shared_markers <- intersect(selected1, selected2)
nonshared1 <- setdiff(selected1, shared_markers)
nonshared2 <- setdiff(selected2, shared_markers)

cat("Shared markers:", paste(shared_markers, collapse = ", "), "\n")
cat("Non-shared in FCS1:", paste(nonshared1, collapse = ", "), "\n")
cat("Non-shared in FCS2:", paste(nonshared2, collapse = ", "), "\n")
```
...
MultiCombineFCS <- function(file_list, marker_list, arcsinh=FALSE) {
  fcs1 <- file_list[[1]]
  fcs2 <- file_list[[2]]
...
for (patient in names(fcs_by_patient)) {
  ...
  selected_files <- patient_files[1:4]
  selected_markers <- marker_list[1:4]
  merged_data <- MultiCombineFCS(selected_files, selected_markers, arcsinh=FALSE)
