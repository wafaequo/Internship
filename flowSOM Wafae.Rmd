...
results <- expand.grid(
  manual_label = unique(expr_all$manual_label),
  marker = markers,
  stringsAsFactors = FALSE
) %>%
  mutate(comparisons = pmap(list(manual_label, marker), function(label, marker) {
    df <- expr_all %>% filter(manual_label == label)
    
    map_dfr(disease_pairs, function(pair) {
      g1 <- df %>% filter(disease == pair[1]) %>% pull(marker)
      g2 <- df %>% filter(disease == pair[2]) %>% pull(marker)
      
      if (length(g1) < 2 || length(g2) < 2) return(NULL)
      
      test <- wilcox.test(g1, g2, exact = FALSE) #statistical test
      delta <- cliff.delta(g1, g2)$estimate #cliff's delta calculation
