...
fcs_margins <- lapply(fcs_scaled, function(ff) {
...
#### STEP 1.4.1 - Scaling to fit bulk data

```{r}
#PARAMETERS
ref <- fcs_cleaned[[3]] #Reference to which we fit the scaling
skip_ch <- c("Original_ID", "TIME") #which channels to skip during scaling

#SCALING FUNCTION
scale_to_reference <- function(target_fcs, reference_fcs, skip_channels = c("Original_ID", "TIME")) {
  target_name <- keyword(target_fcs)[["FILENAME"]] #which file are we scaling

  pd_target <- pData(parameters(target_fcs))
  pd_ref    <- pData(parameters(reference_fcs))
  
  common_channels <- intersect(pd_target$name, pd_ref$name)
  
  #Check if scaling is necessary
  need_scaling <- FALSE
  for (ch_name in common_channels) {
    if (ch_name %in% skip_channels) next
    idx_target <- which(pd_target$name == ch_name)
    idx_ref    <- which(pd_ref$name == ch_name)
    srcR <- as.numeric(keyword(target_fcs, paste0("$P", idx_target, "R"))[[1]])
    refR <- as.numeric(keyword(reference_fcs, paste0("$P", idx_ref, "R"))[[1]])
    if (!isTRUE(all.equal(srcR, refR))) {
      need_scaling <- TRUE
      break
    }
  }
  ...
  message(sprintf("Scaling needed for file '%s'.",
                    target_name))
  e_target <- exprs(target_fcs)
  ...
  for (ch_name in common_channels) {
    ...
    factor <- refR / srcR
    e_target[, idx_target] <- e_target[, idx_target] * factor
    keyword(target_fcs)[[paste0("$P", idx_target, "R")]] <- as.character(refR)
  }
  
  exprs(target_fcs) <- e_target
  target_fcs
}

#APPLYING SCALING TO FULL DISEASE GROUP
fcs_scaled <- lapply(fcs_cleaned, function(target) {
  scale_to_reference(target, ref, skip_channels = skip_ch)
})

names(fcs_scaled) <- names(fcs_cleaned)
```

#### STEP 1.4.2 - Transformations
...
fcs_transformed <- lapply(fcs_scaled, arcsinh_transform, cofactor = 150)
```
...
linear_transform <- function(flow_frame, reference_marker = "KO",
  ...
  present_channels <- intersect(scatter_channels, colnames(expr))
  if (!reference_marker %in% colnames(expr)) {
    warning(paste("Skipping file: missing reference channel:", reference_marker))
    return(flow_frame)
  }
  if (length(present_channels) == 0) {
    warning("No scatter channels present, skipping file")
    return(flow_frame)
  }

  # Apply linear transformation only to present channels
...
#### STEP 1.4.3 - Min-Max Normalization
...
write.csv(QC_data_event_counts_df, file = "D:/Internship/afterQC_event_count_aml.csv", row.names = FALSE)
```
...
for (name in names(fcs_transformed)) {
  ...
  p <- ggcyto(raw_ff, aes(x = `FS-A`, y = `FS-H`)) +
    geom_hex(bins = 128) +
    labs(
      title = paste("FSC_A vs FSC-H - (before gating):", base_name),
      x = "FSC_A", y = "FSC-H"
    ) +
    theme_minimal() +
...
# Give names to fcs_gated based on the file names
names(fcs_gated) <- names(fcs_transformed)

# Now extract event counts
gated_data_event_counts <- sapply(fcs_gated, nrow)

gated_data_event_counts_df <- data.frame(
  Sample = names(fcs_gated),
  Events = gated_data_event_counts
)

# Save to CSV
write.csv(gated_data_event_counts_df,
          file = "D:/Internship/gated_event_count_aml.csv",
          row.names = FALSE)
...
#change name to right disease state
write.csv(single_data_event_counts_df, file = "D:/Internship/preprocessed_event_count_aml.csv", row.names = FALSE)
```
...
```{r}
output_dir <- normalizePath(
  file.path("D:/Internship/data/5. preprocessed dataset", patient_group, "/after_gating/FSC-AvsFSC-H"),
  mustWork = FALSE
)
dir.create(output_dir, showWarnings = FALSE)
...
for (name in names(fcs_singles)) {
  ...
  p <- ggcyto(raw_ff, aes(x = `FS-A`, y = `FS-H`)) +
    geom_hex(bins = 128) +
    labs(
      title = paste("FSC-A vs FSC-H - (after gating):", base_name),
      x = "FSC-A", y = "FSC_H"
    ) +
    theme_minimal() +
    scale_fill_viridis_c(option = "C", direction = -1)
  ...
  ggsave(
    filename = file.path(output_dir, paste0(base_name, "_FSC-A_vs_FSC_H.png")),
    plot = p,
    width = 5, height = 5, dpi = 300
  )
}
...
```{r}
#MANUALLY SETTING THE NAMES OF THE TUBES USING TUBE_MAPS
#names(fcs_singles) <- names(fcs_transformed)
...
#helper function for tube number extraction
...
for (fcs_name in names(fcs_singles)) {
  ...
  tube_key <- paste0("tube ", tube_num)
  
  if (!tube_key %in% names(tube_maps)) {
    warning(paste("No mapping found for", fcs_name))
    next
  }
  
  fcs_obj <- fcs_singles[[fcs_name]]
  map <- tube_maps[[tube_key]]
  
  # Only rename columns that exist in the FCS file
  cols_present <- colnames(fcs_obj@exprs)
  rename_idx <- intersect(names(map), cols_present)
  
  # Update the expression matrix column names
  new_names <- colnames(fcs_obj@exprs)
  new_names[match(rename_idx, new_names)] <- map[rename_idx]
  colnames(fcs_obj@exprs) <- new_names
  
  # Update parameter metadata ($PnS for marker, $PnN for channel if needed)
  param_desc <- pData(parameters(fcs_obj))
  for (i in seq_along(rename_idx)) {
    old <- rename_idx[i]
    new <- map[old]
    
    # update $PnS (marker/desc field)
    param_desc$desc[which(param_desc$name == old)] <- new
    
    # optionally also update $PnN (channel name itself, e.g. FITC -> Ki-67)
    param_desc$name[which(param_desc$name == old)] <- new
  }
  pData(parameters(fcs_obj)) <- param_desc
...
## INTERMISSION: Ki67/Bcl-2 density & Ki67/Bcl-2 positivity

Mestrum et al. (2025) researched the Ki67/Bcl-2 positive fractions. Meaning it filtered on Ki67/Bcl-2 positive cells and then calculates how much of those cells belong to Blast cells, erythroid cells, monocytes or myeloid cells. As this is a binary method, we are not able to mimick these results using FlowSOM. What we however can do is the following:

1.  We establish a Ki67 and Bcl-2 density plot based on MFI: This just displays per node what the Mean Intensity Fluorescence is. This is not correlated to the positive fraction, as it may be that the positive fraction of a celltype is high, while its density is low. Establishing these trees may reveal underlying heterogeneity in Ki67 and Bcl-2 expression across different cell populations, highligh nodes with relatively higher proliferative or anti-apoptotic profiles, and provide insights into exprssion gradients.
2.  We determine Ki67 and Bcl-2 positive and assign a 0 or 1 per event. When we then make the FlowSom Trees we can say what the percentage of Ki67 positive or Bcl-2 positivity is per node.

### STEP I.1:  Ki67 POSITIVE CELLS
...
```{r}
### STEP 1: define function to create a rectangular gate on Tube 1
make_igg_cutoff <- function(fcs, igg_channel) {
  dat <- exprs(fcs)
  igg <- dat[, igg_channel]
  quantile(igg, probs = 0.98, na.rm = TRUE)
}

process_patient <- function(tubes, cutoff_ki67, cutoff_bcl2) {
  lapply(names(tubes), function(name) {
    fcs <- tubes[[name]]
    
    # Skip tube 1 (control tube used to define cutoffs)
    if (grepl("tube 1", name, ignore.case = TRUE)) {
      return(fcs)
    }
    
    dat <- exprs(fcs)
    
    # Add Ki67-pos if Ki67 is present
    if ("Ki-67" %in% colnames(dat)) {
      new_mat <- matrix(as.integer(dat[, "Ki-67"] > cutoff_ki67), ncol = 1)
      colnames(new_mat) <- "Ki67_pos"
      fcs <- fr_append_cols(fcs, new_mat)
    }
    
    # Add Bcl2-pos if Bcl-2 is present
    if ("Bcl-2" %in% colnames(dat)) {
      new_mat <- matrix(as.integer(dat[, "Bcl-2"] > cutoff_bcl2), ncol = 1)
      colnames(new_mat) <- "Bcl2_pos"
      fcs <- fr_append_cols(fcs, new_mat)
    }
    
    fcs
  }) |> setNames(names(tubes))
}

# Process all patients
all_patients_processed <- lapply(fcs_by_patient, function(one_patient) {
  
  # --- identify tube 1 ---
  tube1_name <- grep("tube 1", names(one_patient), ignore.case = TRUE, value = TRUE)
  tube1 <- one_patient[[tube1_name]]
  
  # --- get cutoffs ---
  cutoff_ki67 <- make_igg_cutoff(tube1, "IgG1-1")
  cutoff_bcl2 <- make_igg_cutoff(tube1, "IgG1-2")
  
  # --- process all tubes for this patient ---
  process_patient(one_patient, cutoff_ki67, cutoff_bcl2)
  
}) |> setNames(names(fcs_by_patient))

```

```{r}
# Define output folder
output_dir <- file.path("D:/Internship/data/i. Ki67 and Bcl2 pos", patient_group)
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

# Loop over patients
for (patient in names(all_patients_processed)) {
  cat("Saving processed tubes for patient:", patient, "\n")
  
  patient_tubes <- all_patients_processed[[patient]]
  
  # Save each tube individually
  for (tube_name in names(patient_tubes)) {
    tube_fcs <- patient_tubes[[tube_name]]
    
    output_path <- file.path(output_dir, paste0(tube_name, "_processed.fcs"))
    flowCore::write.FCS(tube_fcs, filename = output_path)
  }
  
  cat("Saved all tubes for patient:", patient, "\n")
}
```
...
patient_group <- "aml"  #healthy BM/mds/aml
fcs_dir <- file.path("D:/Internship/data/i. Ki67 and Bcl2 pos", patient_group)


fcs_file_names <- list.files(path = fcs_dir, pattern = "\\.fcs$", full.names = FALSE)
fcs_file_names <- fcs_file_names[!grepl("tube 1", fcs_file_names, ignore.case = TRUE)]
fcs_paths <- file.path(fcs_dir, fcs_file_names)
...
marker_list <- list(
  c("FSC-A", "SSC-A", "IgG1-", "IgG1-2", "CD13", "CD117", "CD34", "HLA-DR", "CD45","FSC-H"),  #Tube 1
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "CD14", "CD64", "CD13", "CD117", "CD34", "CD10", "CD11b", "HLA-DR", "CD45", "Ki67_pos"),  #Tube 2
  c("FSC-H", "FSC-A", "SSC-A", "Ki-67", "CD105", "CD123", "CD33", "CD117", "CD71", "CD235a", "HLA-DR", "CD45", "Ki67_pos"), #Tube 3
  c("FSC-H", "FSC-A", "SSC-A","Ki-67","CD33", "CD117", "CD36","HLA-DR", "CD45", "Ki67_pos"), #Tube 4
    c("FSC-H", "FSC-A", "SSC-A", "CD16", "CD64", "Bcl-2", "CD13", "CD117", "CD34", "CD10", "CD14", "HLA-DR", "CD45", "Bcl2_pos"), #Tube 5
  c("FSC-H", "FSC-A", "SSC-A", "CD105", "Bcl-2", "CD33", "CD117", "CD71", "CD235a", "HLA-DR", "CD45", "Bcl2_pos"), #Tube 6
  c("FSC-H", "FSC-A", "SSC-A", "Bcl-2", "CD33", "CD117", "CD36","HLA-DR", "CD45", "Bcl2_pos") #Tube 7
) 
...
for (patient in names(fcs_by_patient)) {
  ...
  selected_files <- patient_files
  selected_markers <- marker_list[c(2,3,4,5,6,7)]
...
#set parameters
set.seed(2020)
aggregate_cells <- 720000
```

```{r}
ff <- flowCore::read.FCS(fcs_paths[10], transformation = FALSE)
ff_expr <- exprs(ff)
```
...
In this section of the code, the actual implementation and analysis of flowSOM objects are done. Start from step 1 if you still need to train the fSOM object. When you already have a trained and manually labeled fSOM object start from step 2.

## STEP 1: TRAINING FSOM
...
### STEP 1.1 - LOADING IN DATA + PARAMETERS
...
SOM_x <- 15 
SOM_y <- 15 
n_meta <- 15 #how many metaclusters?
seed <- 2020 
scaling <- FALSE
```

### STEP 1.2 - TRAINING FLOWSOM
...
```{r}
excluded_markers <- c("File", "File_scattered", "Original_ID", "FSC-H", "Ki67_pos", "Bcl2_pos") #which markers do I not include for training?
expr_matrix <- exprs(ff)
selected_markers <- setdiff(colnames(expr_matrix), excluded_markers)

fsom <- FlowSOM(input = ff, scale = scaling, 
                seed = seed, 
                nClus = n_meta, 
                xdim = SOM_x, 
                ydim = SOM_y,
...
```{r}
#saving trained tree
saveRDS(fsom, file = file.path(dir_results, "fsom_15X15meta15_aml.rds"))
```

### STEP 1.3 - FLOWSOM SUMMARY
...
```{r}
FlowSOMmary(fsom, plotFile = file.path(dir_results, "fsom_summary_15x15meta15_aml.pdf"))
```

## STEP 2: EXTRACTING TRAINED FSOM OBJECT (NOT MANUALLY LABELED)

Once you have trained your fsom, you can extract it using the following code to start manual labeling process. 
...
```{r}
#parameters needed for further analysis
patient_group <- 'mds' #healthy BM/mds/aml
dir_results <- file.path("D:/Internship/FlowSOM results", patient_group)
dir.create(dir_results, showWarnings = FALSE)
...
#extracting trained fsom object
fsom <- readRDS("D:/Internship/FlowSOM results/healthy BM/fsom_15X15meta15_healthyBM.rds")
```

## STEP 3: MANUAL LABELING

In order to establish which metaclusters represent which types of cells, it is useful to look at 2D scatterplots.
...
metaclusters_of_interest <- c(1,7) #seq_len(n_meta)
node_of_interest <- NULL

Plot2DScatters(fsom = fsom, channelpairs = channel_pairs, metaclusters = metaclusters_of_interest, clusters = node_of_interest, plotFile = paste0(dir_results, "fsom_2D_myeloblasts_scatterplots.png"), sizePoints=1.5, centers = FALSE, density=FALSE)
```
...
metacluster_groups <- list(
  "Mature Erythroids"     = c(3,4),
  "Immature Erythroids"   = c(2),
  "Mature Myeloids"       = c(14,11,16,17,18),
  "Immature Myeloids"     = c(5,9),
  "Platelets"             = c(),
  "Blasts"                = c(1,7),
  "CD36-/CD71+ cells"     = c(),
  "Eosinophils"           = c(8),
  "Debris"                = c(19),
  "Mature Monocytes"      = c(10),
  "Lymphocytes"           = c(6,13,15),
  "Other"                 = c(20,12)
)
...
saveRDS(fsom, file = file.path(dir_results, "fsom_15X15meta20_mds (2).rds"))
```
...
ggsave(filename = paste0(dir_results, "fsom_tree_15x15meta20_mds_manual.pdf"), plot = p, height = 8.5, width = 13)
```
...
library(future)  # plan for parallelization
```

### STEP 4.1 - LOAD IN MANUALLY LABELED FSOM OBJECT
...
SOM_x <- 15 
SOM_y <- 15 
n_meta <- 20 #how many metaclusters?
seed <- 2020 
scaling <- FALSE

#extracting manually labeled fsom object
fsom <- readRDS("D:/Internship/FlowSOM results/mds/fsom_15X15meta20_mds (2).rds")
```

### STEP 4.2 - EXPRESSION TREES FUNCTIONAL MARKERS
...
ggsave(filename = paste0(dir_results, "fdsom_tree_Bcl2_expression.pdf"), plot = p2, height = 8.5, width = 11)
```

### STEP 4.3 - CELL COUNTS

As AML and MDS are disease states known for an elevated amount of celltypes, it is usefull to investigate the cell percentages and compare this with literature. Thus in this section, cell percentages are calculated for each manually defined celltype.

NOTE: bone marrow aspirates provide only a general overview of bone marrow cellularity. So we can only make a general comparison of cell proportions using reference values from differential counts.

```{r}
#labeling events with celltype
...
In order to see what differences are between metaclusters within the same celltype, it is good to investigate the MFI values per marker per metacluster. This clustering heatmap can also be found in flowSOMmary.

```{r}
#MFI per metacluster per marker
mfi_table <- GetMetaclusterMFIs(fsom)
mfi_df$metacluster <- rownames(mfi_df)

#visualise
pheatmap(mfi_table, scale = "column")
```

### STEP 4.5 - STATISTICAL TESTING

In order to see if differences per celltype can be found between disease states, statistical testing was employed. Wilcoxon Rank-Sum Test are performed with Bonferroni correction.

The intention was to use the Cliff's delta as effect size measure, however during calculation, NA was seen due to integer overflow. Thus probability-weighted sampling was used for the cliff's delta calculations with 1000 iterations and downsampling to 10.000 values. Additionally the Vargha and Delaney’s was calculated.

```{r}
#DATA EXTRACTION
fsom_BM <- readRDS("D:/Internship/FlowSOM results/healthy BM/fsom_12X12meta20_healthyB.rds")
fsom_MDS <- readRDS("D:/Internship/FlowSOM results/mds/fsom_15X15meta20_mds (2).rds")
fsom_AML <- readRDS("D:/Internship/FlowSOM results/aml/fsom_15X15meta20_AML (2).rds")

expr_BM <- as.data.frame(fsom_BM$data)
expr_MDS <- as.data.frame(fsom_MDS$data)
expr_AML <- as.data.frame(fsom_AML$data)

# BM
expr_BM$node <- fsom_BM$map$mapping[,1]
expr_BM$manual_label <- fsom_BM$metaclustering_labels[expr_BM$node]
expr_BM$disease <- "BM"

# MDS
expr_MDS$node <- fsom_MDS$map$mapping[,1]
expr_MDS$manual_label <- fsom_MDS$metaclustering_labels[expr_MDS$node]
expr_MDS$disease <- "MDS"

# AML
expr_AML$node <- fsom_AML$map$mapping[,1]
expr_AML$manual_label <- fsom_AML$metaclustering_labels[expr_AML$node]
expr_AML$disease <- "AML"

expr_all <- bind_rows(expr_BM, expr_MDS, expr_AML) #merge

node_summary <- expr_all %>%
  group_by(disease, manual_label, node) %>%
  summarise(across(c("Ki-67"), \(x) median(x, na.rm = TRUE)), .groups = "drop")

#save median values per node
#write.csv(node_summary, "D:/Internship/FlowSOM results/nodesum_ki_67.csv", row.names = FALSE)
```

```{r}
library(dplyr)

expr_BM_summary <- expr_MDS %>%
  group_by(manual_label) %>%
  summarise(median_Ki67 = median(`Ki-67`, na.rm = TRUE),
            IQR_low = quantile(`Ki-67`, 0.25, na.rm = TRUE),
            IQR_high = quantile(`Ki-67`, 0.75, na.rm = TRUE),
            .groups = "drop")

write.csv(expr_BM_summary,
          "D:/Internship/FlowSOM results/expr_BM_summary.csv",
          row.names = FALSE)
...
# 1) Summarize Ki-67 for BM (median + IQR)
sum_BM <- expr_MDS %>%
  filter(!is.na(`Ki-67`), !is.na(manual_label)) %>%
  group_by(manual_label) %>%
  summarise(
    median = median(`Ki-67`, na.rm = TRUE),
    q1     = quantile(`Ki-67`, 0.25, na.rm = TRUE),
    q3     = quantile(`Ki-67`, 0.75, na.rm = TRUE),
    n      = dplyr::n(),
    .groups = "drop"
  )

selected_types <- c("Blasts","Immature Myeloids", "Mature Myeloids", "Immature Erythroids", "Mature Erythroids", "Mature Monocytes", "Lymphocytes")

sum_BM_selected <- sum_BM %>%
  filter(manual_label %in% selected_types)

sum_BM_selected$manual_label <- factor(
  sum_BM_selected$manual_label,
  levels = selected_types
)

ggplot(sum_BM_selected, aes(x = manual_label, y = median, color = manual_label)) +
  geom_errorbar(aes(ymin = q1, ymax = q3), width = 0.1, linewidth = 1.2) +
  geom_point(size = 4, shape = 15) +
  scale_color_manual(values = celltype_colors) +
  labs(
    y = "Ki-67 expression (median ± IQR)",
    x = "Cell type"
  ) +
...
```{r}
#CELLTYPES WITHIN ONE DISEASE STATE

all_pairs <- combn(unique(df_long$manual_label), 2, simplify = FALSE)

exclude <- c("Platelets", "Debris", "Other","Eosinophils", "CD36-/CD71+ cells")

filtered_pairs <- celltype_pairs[!sapply(celltype_pairs, function(x) any(x %in% exclude))]

celltype_pairs <- unique(lapply(filtered_pairs, sort))

markers <- c("Ki-67") #which markers?

#convert data to long format
df_long <- expr_all %>%
  select(manual_label, disease, all_of(markers)) %>%
  pivot_longer(cols = all_of(markers),
               names_to = "marker",
               values_to = "value")

#Downsampling for cliff's calculation
downsample_preserve_dist <- function(x, max_size = 10000) {
  n <- length(x)
  if(n <= max_size) return(x)

  dens <- density(x, n = max_size) #kernal density estimate for data
  idx <- findInterval(x, dens$x)
  probs <- dens$y[idx]
  probs <- probs / sum(probs)
  
  sample(x, max_size, prob = probs) #weighted sampling according to KDE
}

plan(multisession, workers = parallel::detectCores() - 1)

#Cliff's delta calculation (Bootstrapping)
cliff_delta_boot_parallel <- function(g1, g2, max_size = 10000, n_boot = 1000, ci_level = 0.95) {

  single_delta <- function(i) {
    g1_s <- if(length(g1) > max_size) sample(g1, max_size) else g1
    g2_s <- if(length(g2) > max_size) sample(g2, max_size) else g2
    cliff.delta(g1_s, g2_s)$estimate
  }
  
  #Bootstrap calculations
  deltas <- future_map_dbl(1:n_boot, single_delta, .options = furrr_options(seed = TRUE))
  
  alpha <- 1 - ci_level
  ci_lower <- quantile(deltas, alpha / 2, na.rm = TRUE)
  ci_upper <- quantile(deltas, 1 - alpha / 2, na.rm = TRUE)
  
  list(
    Cliffs_delta = median(deltas, na.rm = TRUE),
    CI_lower = ci_lower,
    CI_upper = ci_upper
  )
}
...
run_stats_within_disease <- function(data, max_size = 10000, n_boot = 1000) {
  map_dfr(celltype_pairs, function(pair) {
    g1 <- filter(data, manual_label == pair[1]) %>% pull(value)
    g2 <- filter(data, manual_label == pair[2]) %>% pull(value)
    
    if(length(g1) < 2 || length(g2) < 2) return(NULL)
    
    cliff_res <- cliff_delta_boot_parallel(g1, g2, max_size, n_boot)
    
    tibble(
      Celltype_1 = pair[1],
      Celltype_2 = pair[2],
      Wilcoxon_p = wilcox.test(g1, g2, exact = FALSE)$p.value,
      VD_A = round(VD.A(g1, g2)$estimate, 2),
      Cliffs_delta = round(cliff_res$Cliffs_delta, 2),
      CI_lower = round(cliff_res$CI_lower, 2),
      CI_upper = round(cliff_res$CI_upper, 2),
      Median_1 = median(g1, na.rm = TRUE),
      Median_2 = median(g2, na.rm = TRUE)
    )
  })
}

results_celltype <- df_long %>%
  group_by(disease, marker) %>%
  group_modify(~ run_stats_within_disease(.x, max_size = 10000, n_boot = 1000)) %>%
  ungroup() %>%
  mutate(
    Bonferroni_p = pmin(Wilcoxon_p * nrow(.), 1),
    Significant = Bonferroni_p < 0.05
  )


#Save the results to a CSV file
write.csv(results_celltype, "D:/Internship/FlowSOM results/manual_label_wilcox_celltypesonediseasetstate_ki-67.csv", row.names = FALSE)
```

```{r}
#CELLTYPES BETWEEN DISEASE STATES

#STATISTICAL ANALYSIS
disease_pairs <- list(
  c("BM", "MDS"),
  c("BM", "AML"),
  c("MDS", "AML")) #which comparison

markers <- c("Bcl-2") #which markers?
...
#STATISTICAL ANALYSIS
run_stats_parallel_boot <- function(data, max_size = 10000, n_boot = 1000) {
  map_dfr(disease_pairs, function(pair) {
    g1 <- filter(data, disease == pair[1]) %>% pull(value)
    g2 <- filter(data, disease == pair[2]) %>% pull(value)
...
results <- df_long %>%
  group_by(manual_label, marker) %>%
  group_modify(~ run_stats_parallel_boot(.x, max_size = 10000, n_boot = 1000)) %>%
  ungroup() %>%
  mutate(
    Bonferroni_p = pmin(Wilcoxon_p * nrow(.), 1),
    Significant = Bonferroni_p < 0.05
...
#Save the results to a CSV file
write.csv(results, "D:/Internship/FlowSOM results/manual_label_wilcox_bootstrap_Bcl-2.csv", row.names = FALSE)
```
