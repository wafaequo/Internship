...
```{r}
library(FlowSOM)
fsom <- readRDS("D:/Internship/FlowSOM results/mds/fsom_15X15meta20_mds.rds")
```
...
metaclusters_of_interest <- c(6,13,15) #seq_len(n_meta)
node_of_interest <- NULL

Plot2DScatters(fsom = fsom, channelpairs = channel_pairs, metaclusters = metaclusters_of_interest, clusters = node_of_interest, plotFile = paste0(dir_results, "fsom_2D_lymph_scatterplot_metaclusters.png"), sizePoints=1.5, centers = FALSE, density=FALSE)
```
...
```{r}
#expression values per manual labeled group
library(dplyr)
library(FlowSOM)
...
# 1. Extract expression data from FlowSOM objects
...
expr_all <- bind_rows(expr_BM, expr_MDS, expr_AML)

node_summary <- expr_all %>%
  group_by(disease, manual_label, node) %>%
  summarise(across(c("Ki-67", "Bcl-2"), mean, na.rm = TRUE), .groups = "drop")
...
markers <- c("Ki-67", "Bcl-2")

# Run comparisons per manual_label and marker
results <- expand.grid(
  ...
  mutate(comparisons = pmap(list(manual_label, marker), function(label, marker) {
    ...
    map_dfr(disease_pairs, function(pair) {
      ...
      delta <- cliff.delta(g1, g2)$estimate
