...
#saving dataframe with which versions and columns names are present per file
fcs_summary_export <- fcs_summary_df %>%
  mutate(column_names = sapply(column_names, function(x) paste(x, collapse = ", ")))

write.csv(fcs_summary_export, "D:/Internship/data/fcs_summary_healthyBM.csv", row.names = FALSE)
...
```{r}
fcs_patients <- fcs_by_patient[48:50]
```
...
for (patient in names(fcs_patients)) {
  cat("Processing patient:", patient, "\n")
  
  patient_files <- fcs_patients[[patient]]
  ...
  # Select tubes to merge (change indices as needed)
  selected_files <- patient_files[c(2:7)]

  # If marker_list is per-patient:
  # selected_markers <- marker_list[[patient]][c(2:7)]
  # If marker_list is global:
  selected_markers <- marker_list[c(2:7)]
...
```{r}
ff_patient <- read.FCS("D:/Internship/data/6. merged dataset/healthy BM/Femoral head 1_merged.fcs")
expr_ff <- exprs(ff_patient)
```
...
ff <- read.FCS("D:/Internship/data/7. aggregated files/healthy BM/all_tubes_aggregated_everything.fcs")
colnames(ff)
```
...
```{r}
saveRDS(fsom, file = file.path(dir_results, "fsom_15X15meta20_healthyBM.rds"))
```
...
ggsave(filename = paste0(dir_results, "fsom_tree_20x20meta15_healthyBM.pdf"), plot = p, height = 8.5, width = 11)
```
...
```{r}
FlowSOMmary(fsom, plotFile = paste0(dir_results, "fsom_summary_20x20meta15_healthyBM.pdf"))
```
...
```{r}
fsom <- readRDS("D:/Internship/FlowSOM results/healthy BM/everything/(15x15) 20 meta/fsom_15X15meta20_healthyBM.rds")
```
...
```{r}
#SCATTERPLOTS
channel_pairs = list(c("CD45", "SSC-A"), c("CD13", "SSC-A")) 

metaclusters_of_interest <- c(4,7) #seq_len(n_meta)
clusters_of_interest <- NULL

Plot2DScatters(fsom = fsom, channelpairs = channel_pairs, metaclusters = metaclusters_of_interest, clusters = clusters_of_interest, plotFile = paste0(dir_results, "fsom_2D_lymph_scatterplot_metaclusters.png"), centers = FALSE, density=FALSE)
```
