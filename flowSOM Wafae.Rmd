...
-   for MDS patients: 40.000 x 18 = 720.000
...
excluded_markers <- c("File", "File_scattered", "Original_ID", "FSC-H") #which markers do I not include for training?
...
```{r}
#expression trees
p2 <- PlotMarker(fsom = fsom, marker = 'Bcl-2', maxNodeSize=1, equalNodeSize=TRUE)
ggsave(filename = paste0(dir_results, "fsom_tree_Bcl2_expression.pdf"), plot = p2, height = 8.5, width = 11)
```
...
In order to see if differences per celltype can be found between disease states, statistical testing was employed. Wilcoxon Rank-Sum Test are performed with Benferroni correction.
...
write.csv(node_summary, "D:/Internship/FlowSOM results/nodesum_Ki67.csv", row.names = FALSE)
```

```{r}
#CLIFFS DELTA SAMPLING ONZIN
...
write.csv(results, "D:/Internship/FlowSOM results/manual_label_wilcox_bootstrap.csv", row.names = FALSE)
```
